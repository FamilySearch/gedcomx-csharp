...
namespace FamilySearch.Api
{
    public class FamilySearchCollectionState : CollectionState
    {
        ...
        private FamilySearchCollectionState(Uri uri, IRestClient client, FamilySearchStateFactory stateFactory)
            : this(new RestRequest().Accept(MediaTypes.GEDCOMX_JSON_MEDIA_TYPE).Build(uri, Method.GET), client, stateFactory)
        {
        }

        private FamilySearchCollectionState(IRestRequest request, IRestClient client, FamilySearchStateFactory stateFactory)
            : this(request, client.Execute(request), client, null, stateFactory)
        {
        }

        protected internal FamilySearchCollectionState(IRestRequest request, IRestResponse response, IRestClient client, String accessToken, FamilySearchStateFactory stateFactory)
