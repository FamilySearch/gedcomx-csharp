...
public static class XmlAssertions
{
    ...
    public static void ShouldContain(this string result, SourceReference sourceReference)
    {
        Assert.That(result, Does.Contain("<sourceReference "));
        Assert.That(result, Does.Contain("xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\""));
        Assert.That(result, Does.Contain("xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\""));
        Assert.That(result.Contains("description="), Is.EqualTo(sourceReference.DescriptionRef != null));
        Assert.That(result.Contains("<attribution"), Is.EqualTo(sourceReference.Attribution != null));
        Assert.That(result.Contains("<qualifier "), Is.EqualTo(sourceReference.AnyQualifiers()));
        Assert.That(result.Contains("<tags "), Is.EqualTo(sourceReference.AnyTags()));
        result.ShouldContain(sourceReference as HypermediaEnabledData);
    }
